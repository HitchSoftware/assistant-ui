...
- **Styled output** — the vocabulary stylesheet is mounted from `@hitchsoftware/assistant-ui-ui` through the monorepo's tsconfig alias, the way every in-repo example consumes that package; in your own app the same CSS arrives through `shadcn add generative-ui`
