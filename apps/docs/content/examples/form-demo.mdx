...
The example uses `useAssistantForm` from `@hitchsoftware/assistant-ui-react-hook-form` to connect the AI with form state. This hook replaces the standard `useForm` and automatically registers `set_form_field` and `submit_form` tools with the assistant.
