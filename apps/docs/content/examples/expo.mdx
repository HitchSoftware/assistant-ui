...
A full-featured React Native chat application built with Expo and `@hitchsoftware/assistant-ui-react-native`. It demonstrates drawer-based thread management, streaming OpenAI responses, and a polished native UI with dark mode support.
...
The runtime uses `useChatRuntime` from `@hitchsoftware/assistant-ui-react-ai-sdk` with an `AssistantChatTransport` that connects to a chat API endpoint:
