...
We're launching `@hitchsoftware/assistant-ui-react-native` — native React Native bindings that bring the full assistant-ui experience to mobile.
...
```sh
npx expo install @hitchsoftware/assistant-ui-react-native @hitchsoftware/assistant-ui-react-ai-sdk
```

The API mirrors the web version closely. If you've built with `@hitchsoftware/assistant-ui-react`, you already know how to use it.
...
We're launching `@hitchsoftware/assistant-ui-react-ink` — terminal UI bindings powered by [Ink](https://github.com/vadimdemedes/ink), the React renderer for CLIs.
...
- **Markdown rendering.** Ship with `@hitchsoftware/assistant-ui-react-ink-markdown` for formatted headings, code blocks with syntax highlighting, tables, and lists — all in your terminal.
...
We're launching `@hitchsoftware/assistant-ui-cloud-ai-sdk` — a standalone package that adds full thread persistence and management to any Vercel AI SDK application with a single hook change. No providers, no context, no runtime objects.
...
```
@hitchsoftware/assistant-ui-react          → Web (React + Radix UI)
@hitchsoftware/assistant-ui-react-native   → Mobile (React Native)
@hitchsoftware/assistant-ui-react-ink      → Terminal (Ink)
```
