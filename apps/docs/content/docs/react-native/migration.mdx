...
description: Migrate an existing @hitchsoftware/assistant-ui-react app to React Native.
---

If you already have an assistant-ui web app, most of your code transfers directly. The runtime core is shared between `@hitchsoftware/assistant-ui-react` and `@hitchsoftware/assistant-ui-react-native` via `@hitchsoftware/assistant-ui-core` — only the UI layer changes.

## What stays the same

- **Runtime setup** — `useChatRuntime`, `useLocalRuntime`, `ChatModelAdapter`, and all runtime options work identically.
- **AI SDK integration** — `@hitchsoftware/assistant-ui-react-ai-sdk` works with React Native. Your `useChatRuntime` + `AssistantChatTransport` setup transfers directly.
...
| Web (`@hitchsoftware/assistant-ui-react`) | React Native (`@hitchsoftware/assistant-ui-react-native`) |
|---|---|
...
| `@hitchsoftware/assistant-ui-ui` (shadcn components) | Build your own native UI with primitives |
...
<InstallCommand expo={["@hitchsoftware/assistant-ui-react-native"]} />
...
This is the main migration effort. Web components (`Thread`, `ThreadList` from `@hitchsoftware/assistant-ui-ui`) don't render in React Native. Replace them with native primitives:
