...
Primitives are accessed via namespace imports (e.g. `ThreadPrimitive.Root`, `ComposerPrimitive.Input`), matching the pattern used in `@hitchsoftware/assistant-ui-react`.

Many primitives share their core logic with `@hitchsoftware/assistant-ui-react` via `@hitchsoftware/assistant-ui-core/react` — only the UI layer (View/Pressable/Text vs DOM elements) differs.
...
Renders a single message at a specific index in the thread. Provides message context so that `useAuiState((s) => s.message)` works inside the rendered component. This component is shared from `@hitchsoftware/assistant-ui-core/react`.
...
Renders all suggestions using the provided component configuration. This component is shared from `@hitchsoftware/assistant-ui-core/react`.
...
Renders a single suggestion at the specified index, providing suggestion context. This component is shared from `@hitchsoftware/assistant-ui-core/react`.
...
```tsx
import { AuiIf } from "@hitchsoftware/assistant-ui-react-native";
...
```tsx
import { ComposerPrimitive } from "@hitchsoftware/assistant-ui-react-native";
```
...
```tsx
import { MessagePrimitive } from "@hitchsoftware/assistant-ui-react-native";
```
...
Renders a single message part at the specified index, providing part context. Useful for building custom part layouts outside of `MessagePrimitive.Parts`. This component is shared from `@hitchsoftware/assistant-ui-core/react`.
...
```tsx
import { AttachmentPrimitive } from "@hitchsoftware/assistant-ui-react-native";
```
...
```tsx
import { BranchPickerPrimitive } from "@hitchsoftware/assistant-ui-react-native";
```
...
```tsx
import { ThreadListPrimitive } from "@hitchsoftware/assistant-ui-react-native";
```
...
```tsx
import { ThreadListItemPrimitive } from "@hitchsoftware/assistant-ui-react-native";
```
...
Renders the thread title text. Falls back to the provided fallback when title is empty. This component is shared from `@hitchsoftware/assistant-ui-core/react`.
...
```tsx
import { SuggestionPrimitive } from "@hitchsoftware/assistant-ui-react-native";
```

Primitives for rendering suggestions. Use inside a `SuggestionByIndexProvider` (from `@hitchsoftware/assistant-ui-core/react`).
...
```tsx
import { ChainOfThoughtPrimitive } from "@hitchsoftware/assistant-ui-react-native";
```
...
Renders the parts within a chain of thought. Shared from `@hitchsoftware/assistant-ui-core/react`.
