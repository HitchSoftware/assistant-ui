...
`humanTool` is imported from `@hitchsoftware/assistant-ui-react`. See [Tool UI → Human-in-the-loop](/docs/tools/tool-ui#user-input-collection) for the full pattern.
...
import { useAuiToolOverrides } from "@hitchsoftware/assistant-ui-react";
...
import { defineToolkit, defineMcpToolkit } from "@hitchsoftware/assistant-ui-react";
