...
The `$type` tree your assistant renders in the browser is plain JSON, so it does not have to stay in the browser. `@hitchsoftware/assistant-ui-react-generative-ui/slack` converts the same tree into [Block Kit](https://docs.slack.dev/reference/block-kit/blocks) JSON, decodes the interactions Slack posts back, and parses Block Kit payloads into the tree.
...
import { toSlackBlocks } from "@hitchsoftware/assistant-ui-react-generative-ui/slack";
...
```ts
import { decodeBlockAction } from "@hitchsoftware/assistant-ui-react-generative-ui/slack";
