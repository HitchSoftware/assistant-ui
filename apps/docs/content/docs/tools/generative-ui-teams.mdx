...
The `$type` tree your assistant renders in the browser is plain JSON, so it does not have to stay in the browser. `@hitchsoftware/assistant-ui-react-generative-ui/teams` converts the same tree into an [Adaptive Card](https://adaptivecards.io/explorer/) and decodes the submit payload a Teams bot receives back.
...
```ts
import { toAdaptiveCard } from "@hitchsoftware/assistant-ui-react-generative-ui/teams";
...
```ts
import { decodeSubmitData } from "@hitchsoftware/assistant-ui-react-generative-ui/teams";
