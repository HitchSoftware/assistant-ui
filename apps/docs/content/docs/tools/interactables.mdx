...
```tsx
import { unstable_useInteractable } from "@hitchsoftware/assistant-ui-react";
import { z } from "zod";
...
import { unstable_injectInteractableContext as injectInteractableContext } from "@hitchsoftware/assistant-ui-react-ai-sdk";
...
Each user message carries its state snapshots at `metadata.custom.interactables`. For other backends (LangGraph, Mastra, a custom runtime), build the equivalent injection with the two helpers exported from `@hitchsoftware/assistant-ui-react`:
...
```ts
import { type Unstable_InteractableSnapshotEntry } from "@hitchsoftware/assistant-ui-react";
