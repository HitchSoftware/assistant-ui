...
Register the `present` frontend tool from `@hitchsoftware/assistant-ui-react-generative-ui`; incoming surfaces then render with the default vocabulary:
...
import { useAgUiSendA2uiAction } from "@hitchsoftware/assistant-ui-react-ag-ui";
import {
  JSONGenerativeUI,
