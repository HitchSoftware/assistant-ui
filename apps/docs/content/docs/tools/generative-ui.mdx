---
title: Generative UI
description: Let the model compose an interface at runtime from a component vocabulary you ship, using the present tool from @hitchsoftware/assistant-ui-react-generative-ui.
platforms: ["react"]
---
...
<InstallCommand npm={["@hitchsoftware/assistant-ui-react-generative-ui"]} />
...
Vite and TanStack Start use `aui()` from `@hitchsoftware/assistant-ui-vite`; Expo and bare React Native use `withAui` from `@hitchsoftware/assistant-ui-metro`.
...
- `@hitchsoftware/assistant-ui-react-generative-ui/ir` holds the tree types, normalization, and token enums.
- `@hitchsoftware/assistant-ui-react-generative-ui/slack` converts the tree into Block Kit and decodes `block_actions` back into `$action`. See [Generative UI on Slack](/docs/tools/generative-ui-slack).
- `@hitchsoftware/assistant-ui-react-generative-ui/teams` converts it into an Adaptive Card and decodes the submit payload. See [Generative UI on Microsoft Teams](/docs/tools/generative-ui-teams).
- `@hitchsoftware/assistant-ui-react-generative-ui/a2ui` consumes [A2UI surfaces over AG-UI](/docs/tools/a2ui).
