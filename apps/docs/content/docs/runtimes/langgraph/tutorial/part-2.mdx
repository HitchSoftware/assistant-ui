...
import { AuiConfig, AuiProvider, defineToolkit, Tools, useAui } from "@hitchsoftware/assistant-ui-react";
...
import { ToolCallMessagePartComponent } from "@hitchsoftware/assistant-ui-react";
...
The `Thread` component from `@hitchsoftware/assistant-ui-ui` already includes a built-in `ToolFallback` and `MarkdownText`, so no additional configuration is needed.
