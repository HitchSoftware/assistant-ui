...
<Callout type="warn">
AI SDK v4 is a legacy version. New projects should use [AI SDK v7](/docs/runtimes/ai-sdk/v7). v4 integrations use the older `@hitchsoftware/assistant-ui-react-data-stream` package, not `@hitchsoftware/assistant-ui-react-ai-sdk`.
</Callout>

## Why legacy

Vercel ships AI SDK majors roughly yearly; v4 is three majors behind v7. The current `@hitchsoftware/assistant-ui-react-ai-sdk` package targets the current AI SDK major (v7) exclusively, so v4 users plug into [`@hitchsoftware/assistant-ui-react-data-stream`](/docs/runtimes/custom/data-stream) instead — the same data stream protocol that v4's `toDataStreamResponse()` emits.
...
<InstallCommand npm={["@hitchsoftware/assistant-ui-react", "@hitchsoftware/assistant-ui-react-data-stream", "ai@^4"]} />
...
| Runtime package | `@hitchsoftware/assistant-ui-react-data-stream` | `@hitchsoftware/assistant-ui-react-ai-sdk` |
...
1. Swap `@hitchsoftware/assistant-ui-react-data-stream` for `@hitchsoftware/assistant-ui-react-ai-sdk`.
...
An older release line of `@hitchsoftware/assistant-ui-react-ai-sdk` supported AI SDK v4 directly; its last release was `0.10.16`. It is no longer maintained and has no upgrade path; if you are starting a new v4 project, use `@hitchsoftware/assistant-ui-react-data-stream` instead.
