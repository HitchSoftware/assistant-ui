...
| API | Package | Notes |
| --- | --- | --- |
| `unstable_createMessageConverter` | `@hitchsoftware/assistant-ui-react` | Message-format converter used by AssistantTransport and DataStream. |
| `unstable_humanToolNames` | `@hitchsoftware/assistant-ui-react` | Tool names that pause the run until a result is added via `addResult`. Only available on LocalRuntime; not supported in DataStream. |
| `unstable_threadListAdapter` | `@hitchsoftware/assistant-ui-react-langgraph` | LangGraph thread-list adapter slot on `useLangGraphRuntime`. |
| `unstable_createLangGraphStream` | `@hitchsoftware/assistant-ui-react-langgraph` | End-to-end cancellation primitive. |
