...
This works out of the box with `@hitchsoftware/assistant-ui-react-ai-sdk`. `ModelSelector.Root` performs no registration; it is purely presentational, with controlled and uncontrolled props for the value, effort, and open state.
