...
<Callout type="info">
  `@hitchsoftware/assistant-ui-react-streamdown` is an **alternative** to `@hitchsoftware/assistant-ui-react-markdown`. Choose based on your needs:
...
<PackageManagerTabs packages={["@hitchsoftware/assistant-ui-react-streamdown", "streamdown"]} />
...
If you're migrating from `@hitchsoftware/assistant-ui-react-markdown`, your existing `SyntaxHighlighter` and `CodeHeader` components still work:
...
```tsx
import { useIsStreamdownCodeBlock } from "@hitchsoftware/assistant-ui-react-streamdown";
...
```tsx
import { useStreamdownPreProps } from "@hitchsoftware/assistant-ui-react-streamdown";
