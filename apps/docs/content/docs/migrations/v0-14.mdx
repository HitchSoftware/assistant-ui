...
`toAISDKTools` and `getEnabledTools` (re-exported from `@hitchsoftware/assistant-ui-react`) have been removed. Both wrapped functionality that's now exposed directly by `assistant-stream`:

```ts
// Before
import { toAISDKTools } from "@hitchsoftware/assistant-ui-react";
