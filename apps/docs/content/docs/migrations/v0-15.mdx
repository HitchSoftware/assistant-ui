...
`AuiConfig` is exported from `@hitchsoftware/assistant-ui-store` and re-exported from `@hitchsoftware/assistant-ui-react`, `@hitchsoftware/assistant-ui-react-native`, and `@hitchsoftware/assistant-ui-react-ink`.
