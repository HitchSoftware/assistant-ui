...
<Callout type="info">
This is an integration guide, not a runtime adapter. assistant-ui does not ship a `@hitchsoftware/assistant-ui-react-mastra` package. You wire up Mastra through the standard [AI SDK runtime](/docs/runtimes/ai-sdk/v7) by routing your API endpoint through Mastra's agent stream.
</Callout>
...
Both use the same client-side `useChatRuntime` from [`@hitchsoftware/assistant-ui-react-ai-sdk`](/docs/runtimes/ai-sdk/v7). The only difference is where the Mastra agent lives.
