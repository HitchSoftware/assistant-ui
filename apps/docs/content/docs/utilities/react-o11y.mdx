...
<Callout type="warn">
`@hitchsoftware/assistant-ui-react-o11y` is experimental. The API may change without notice.
</Callout>

`@hitchsoftware/assistant-ui-react-o11y` provides headless, Radix-style primitives for rendering observability spans as collapsible, indented hierarchical UIs. Use it to build trace inspectors, waterfall debug panels, or LLM call timelines on top of your own span data source.
...
<InstallCommand npm={["@hitchsoftware/assistant-ui-react-o11y"]} />
...
import { AuiConfig, AuiProvider } from "@hitchsoftware/assistant-ui-store";
