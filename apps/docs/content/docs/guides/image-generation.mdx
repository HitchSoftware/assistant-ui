...
Image generation needs no dedicated primitive. Generate the image wherever you already run model calls (a route handler or a tool), store the result as an `ImageMessagePart`, and render it with the `@hitchsoftware/assistant-ui-ui` `Image` component.
...
The `Image` component in `@hitchsoftware/assistant-ui-ui` handles the render states for you:
...
```tsx
import { Image } from "@hitchsoftware/assistant-ui-ui";
