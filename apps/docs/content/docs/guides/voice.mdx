...
Implement `RealtimeVoiceAdapter` to connect any voice provider. The contract lives in `@hitchsoftware/assistant-ui-core` and is re-exported from `@hitchsoftware/assistant-ui-react`:
...
These hooks are exported from `@hitchsoftware/assistant-ui-react` and read the active voice session on the current thread.
...
```tsx
import { useVoiceState } from "@hitchsoftware/assistant-ui-react";
...
```tsx
import { useVoiceVolume } from "@hitchsoftware/assistant-ui-react";
...
```tsx
import { useVoiceControls } from "@hitchsoftware/assistant-ui-react";
...
```tsx
import { useVoiceState, useVoiceControls } from "@hitchsoftware/assistant-ui-react";
