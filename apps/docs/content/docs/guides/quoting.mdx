...
```tsx
import { useMessageQuote } from "@hitchsoftware/assistant-ui-react";
...
Set or clear quotes via `useAui` from `@hitchsoftware/assistant-ui-react`. Call `aui.thread.composer().setQuote()` when your component is rendered outside of a specific thread context, or `aui.composer.setQuote()` when it is rendered inside a thread:
