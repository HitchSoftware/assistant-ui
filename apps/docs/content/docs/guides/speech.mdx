...
The adapter has a single method. The type lives in `@hitchsoftware/assistant-ui-core` and is re-exported from `@hitchsoftware/assistant-ui-react`:

```tsx
import type { SpeechSynthesisAdapter } from "@hitchsoftware/assistant-ui-react";
