...
```tsx
import { WebSpeechDictationAdapter } from "@hitchsoftware/assistant-ui-react";
...
Custom providers implement the same contract. The interface is defined in `@hitchsoftware/assistant-ui-core` and re-exported from `@hitchsoftware/assistant-ui-react`:
...
The dictation trigger is `ComposerPrimitive.Dictate`. Pair it with `ComposerPrimitive.StopDictation` to end the session. Both live under `ComposerPrimitive` in `@hitchsoftware/assistant-ui-react`.

```tsx
import { AuiIf, ComposerPrimitive } from "@hitchsoftware/assistant-ui-react";
