...
assistant-ui works in Electron through `@hitchsoftware/assistant-ui-react`. The renderer is a React DOM environment, so there is no separate `@hitchsoftware/assistant-ui-electron` package to install. The Electron-specific decision is where model requests run and how the renderer reaches them.
