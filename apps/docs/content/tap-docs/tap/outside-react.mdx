...
`@hitchsoftware/assistant-ui-store` bridges them into React.
...
[API reference](/tap/docs/tap/api-reference#createtaproot) for the full
contract.
...
<Callout type="warn">
`@hitchsoftware/assistant-ui-store` bridges resources into React with `useSyncExternalStore`, so
