...
assistant-ui in a Nuxt 4 app: `@hitchsoftware/assistant-ui-vue` primitives on the client, streaming from a Nitro server route via the AI SDK.
...
- `nuxt.config.ts` aliases `react` to `@hitchsoftware/assistant-ui-tap/standalone-shim`, so the app runs without React installed.
- Messages render as plain text. `@hitchsoftware/assistant-ui-vue` has no markdown renderer yet.
