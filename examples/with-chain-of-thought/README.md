...
The route emits structured `source-url` chunks; `@hitchsoftware/assistant-ui-react-ai-sdk` converts them to `source` message parts that the `Sources` component renders as badges. They are not URLs scraped from assistant text.
