...
Most apps reach `assistant-stream` indirectly through `@hitchsoftware/assistant-ui-react-ai-sdk` or `@hitchsoftware/assistant-ui-react-data-stream`, which handle the wire format for you. Install it directly when you are building a custom backend or a new integration package.
...
```typescript
import { createAssistantStreamResponse } from "@hitchsoftware/assistant-stream";
