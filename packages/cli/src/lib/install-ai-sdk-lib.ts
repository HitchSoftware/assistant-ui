...
export default async function installAiSdkLib(): Promise<void> {
  await installPackageIfNeeded({
    ...
    importPatterns: ["@hitchsoftware/assistant-ui-react-ai-sdk"],
    promptMessage:
      "AI SDK imports were added but @hitchsoftware/assistant-ui-react-ai-sdk is not installed. Do you want to install it? (Y/n) ",
    skipMessage:
